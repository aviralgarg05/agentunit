...
### Added
- Dedicated CI workflow (`ci.yml`) running lint, tests (matrix), build, and docs checks for contributions
- Example AgentUnit workflow now manual (`workflow_dispatch`) to avoid accidental runs in this repo

### Fixed
- Aligned package version metadata to 0.7.0 for consistency with `pyproject.toml`
