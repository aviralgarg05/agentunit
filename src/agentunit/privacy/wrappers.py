...
class PrivateDatasetWrapper:
    ...
    def __init__(
        self, dataset: list[DatasetCase], enable_pii_masking: bool = True, k_anonymity: int = 3
    ):
    ...
    def _mask_case(self, case: DatasetCase) -> DatasetCase:
        ...
        return DatasetCase(
            id=self._anonymize_id(case.id), query=masked_query, metadata=masked_metadata or None
        )
