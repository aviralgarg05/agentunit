...
try:
    ...
    class np:
        ...
        def std(data, ddof=0):
            ...
            variance = sum((x - mean_val) ** 2 for x in data) / (
                len(data) - ddof if ddof else len(data)
            )
            return variance**0.5
...
class BootstrapCI:
    ...
    def estimate(
        self, data: list[float], statistic: Callable[[list[float]], float] = np.mean
    ) -> ConfidenceInterval:
...
class StatisticalTest:
    ...
    def t_test(
        self, sample1: list[float], sample2: list[float], paired: bool = False
    ) -> TestResult:
        """Perform t-test.
        ...
        if paired:
            if n1 != n2:
                msg = "Paired samples must have same length"
                raise ValueError(msg)

            differences = [a - b for a, b in zip(sample1, sample2, strict=False)]
    ...
    def mann_whitney_u(self, sample1: list[float], sample2: list[float]) -> TestResult:
        ...
        return TestResult(
            ...
            significant=p_value < self.alpha,
            alpha=self.alpha,
            details={"median1": np.median(sample1), "median2": np.median(sample2)},
        )
...
class MetricAggregator:
    ...
    @staticmethod
    def aggregate(values: list[float], methods: list[str] | None = None) -> dict[str, float]:
...
class SignificanceAnalyzer:
    ...
    def analyze_difference(
        self, baseline: list[float], treatment: list[float], metric_name: str = "metric"
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._generate_recommendation(
                statistically_significant, practically_significant, difference
            ),
        }

    def _generate_recommendation(self, stat_sig: bool, pract_sig: bool, difference: float) -> str:
