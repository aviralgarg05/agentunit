...
from .statistics import SignificanceAnalyzer
...
if TYPE_CHECKING:
    ...
    from agentunit.metrics.base import MetricResult
...
class VersionComparator:
    ...
    def __init__(self, significance_analyzer: SignificanceAnalyzer | None = None):
    ...
    async def compare_versions(
        ...
        for i in range(n_runs):
            ...
            baseline_results.append(
                RunResult(version="baseline", config={}, metric_results=[], metadata={"run_id": i})
            )
        ...
        for i in range(n_runs):
            treatment_results.append(
                RunResult(version="treatment", config={}, metric_results=[], metadata={"run_id": i})
            )
        ...
        for metric_name in metrics:
            ...
            if baseline_values and treatment_values:
                analysis = self.analyzer.analyze_difference(
                    baseline_values, treatment_values, metric_name
                )
...
class ConfigurationComparator:
    ...
    def compare_configs(
        ...
        for i, config in enumerate(configs):
            ...
            results[config_id] = {"config": config, "metrics": {}}
    ...
    def _find_best_config(self, results: dict[str, Any], metrics: list[str]) -> str | None:
    ...
    def _rank_by_metrics(self, results: dict[str, Any], metrics: list[str]) -> dict[str, list[str]]:
...
class ABTestRunner:
    ...
    def __init__(self, min_sample_size: int = 30, alpha: float = 0.05, power: float = 0.8):
    ...
    async def run_ab_test(
        ...
        # Analyze results
        analysis = self.analyzer.analyze_difference(results_a, results_b, primary_metric)
        ...
        return {
            ...
            "sample_sizes": {"variant_a": len(results_a), "variant_b": len(results_b)},
        }

    def _should_stop_early(self, results_a: list[float], results_b: list[float]) -> bool:
...
class RegressionDetector:
    ...
    def __init__(self, regression_threshold: float = 0.05, min_effect_size: float = 0.2):
    ...
    def detect_regression(
        ...
        """
        analysis = self.analyzer.analyze_difference(baseline_results, new_results, metric_name)
        ...
        if higher_is_better:
            regression_condition = (
                ...
                and difference < 0
                and abs(percent_change) > self.regression_threshold * 100
            )
        else:
            regression_condition = (
                analysis["statistically_significant"]
        ...
        return {
            ...
            "recommendation": (
                f"BLOCK: {severity} regression detected"
                if is_regression
