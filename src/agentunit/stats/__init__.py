...
class StatisticalAnalyzer:
    ...
    def confidence_interval(
        ...
        if n > 30:
            ...
            n_keys = sorted([k[1] for k in t_values.keys() if k[0] == conf_key])
    ...
    def wilcoxon_signed_rank(
        ...
        w_plus = sum(r for r, (d, _) in zip(ranks, nonzero) if d > 0)
        w_minus = sum(r for r, (d, _) in zip(ranks, nonzero) if d < 0)
    ...
    def mcnemar_test(
        ...
        b = sum(1 for a, b in zip(successes_a, successes_b) if a and not b)  # A correct, B wrong
        c = sum(1 for a, b in zip(successes_a, successes_b) if not a and b)  # A wrong, B correct
