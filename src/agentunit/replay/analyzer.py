...
from agentunit.replay.recorder import RecordedSession, SessionRecorder
...
class SessionAnalyzer:
    ...
    def detect_loops(self) -> list[dict[str, Any]]:
        ...
        for i in range(len(steps) - window_size * 2):
            ...
            window2 = [s.type + str(s.content) for s in steps[i + window_size : i + window_size * 2]]
    ...
    def summarize(self) -> dict[str, Any]:
        ...
        return {
            ...
            ).total_seconds() if self.session.end_time else 0.0,
            "total_steps": len(self.session.steps),
