...
exit codes:
  ...
  2    Configuration or usage error
    ...
    result = run_suite(
        scenarios, metrics=args.metrics, otel_exporter=args.otel_exporter, seed=args.seed
    )
...
def _load_scenarios(target: str) -> list[Scenario]:
    ...
    if path.exists():
        if path.suffix != ".py":
            msg = "Only Python files are supported at the moment"
            raise SystemExit(msg)
        spec = importlib.util.spec_from_file_location(path.stem, path)
        if spec is None or spec.loader is None:
            msg = f"Unable to import module from {path}"
            raise SystemExit(msg)
    ...
    if suite is None:
        msg = "Module must export 'suite' list or 'create_suite' callable"
        raise SystemExit(msg)
    scenarios = list(suite)
    if not all(isinstance(item, Scenario) for item in scenarios):
        msg = "Suite must contain Scenario instances"
        raise SystemExit(msg)
    return scenarios
