...
class BenchmarkRunner:
    ...
    def __init__(self, leaderboard_config: LeaderboardConfig | None = None):
    ...
    def run_gaia(
        ...
        gaia = GAIABenchmark(dataset_path=Path(dataset_path) if dataset_path else None, level=level)
        ...
        if submit and self.submitter:
            submission = gaia.format_submission(
                results, model_name=self.leaderboard_config.model_name
            )
            self.submitter.submit(submission["results"], benchmark_name="gaia")
    ...
    def run_arena(
        self,
        dataset_path: str | None = None,
        task_type: ArenaTaskType | None = None,
        split: str = "test",
        submit: bool = False,
    ) -> BenchmarkResult:
        ...
        arena = AgentArenaBenchmark(
            dataset_path=Path(dataset_path) if dataset_path else None, task_type=task_type
        )
        ...
        # Submit to leaderboard if requested
        if submit and self.submitter:
            self.submitter.submit(results, benchmark_name="agent_arena")
