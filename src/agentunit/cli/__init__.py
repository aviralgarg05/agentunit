...
from agentunit.production.monitoring import BaselineMetrics
...
class CLIConfig:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        """
        self.config_path = (
            Path(config_path) if config_path else Path.home() / ".agentunit" / "config.json"
        )
    ...
    def _create_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "monitoring": {"platform": "none", "credentials": {}},
            "logging": {
                "level": "INFO",
...
class AdapterFactory:
    ...
    def _create_ag2_adapter(config: CLIConfig, **kwargs):
        ...
        return type(
            "MockAG2Adapter",
            (),
            {"model_config": ag2_config.get("model_config", {}), "adapter_type": "autogen_ag2"},
        )()
...
@click.option(
    "--pattern",
    type=click.Choice(["hierarchical", "peer_to_peer", "swarm", "federation", "mesh"]),
    help="Orchestration pattern",
)
@click.option(
    "--communication",
    type=click.Choice(
        ["direct_message", "broadcast", "publish_subscribe", "event_driven", "shared_memory"]
    ),
...
@click.option(
    ...
    required=True,
    help="Path to scenario definition file",
)
@click.option("--adapter", help="Adapter type to use")
@click.pass_context
...
@click.option(
    "--platform",
    type=click.Choice(["langsmith", "agentops", "wandb", "langfuse", "phoenix"]),
    required=True,
    help="Monitoring platform",
)
@click.pass_context
...
def detect_drift(ctx, platform, threshold):
    ...
    try:
        ...
        if not baseline_data:
            click.echo(
                f"❌ No baseline found for {platform}. Run 'establish-baseline' first.", err=True
            )
            sys.exit(1)
